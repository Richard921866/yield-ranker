  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        {children}
        <ArrowUpDown className={`h-3 w-3 ${isActive ? "text-primary" : "text-slate-400"}`} />
      </div>
    </Button>
  );
};
...
export const CEFTable = ({
  ...
  const sortedCEFs = useMemo(() => {
    ...
    const sorted = [...cefs].sort((a, b) => {
      ...
      const textFields: (keyof CEF)[] = ["symbol", "issuer", "description", "navSymbol", "openDate", "dividendHistory"];
        ...
        <table className="w-full caption-bottom text-xs min-w-max">
          <thead className="sticky top-0 z-50 bg-slate-50 shadow-sm border-b-2 border-slate-200">
            <tr className="bg-slate-50">
              <th colSpan={18} className="h-7 px-1.5 text-center align-middle font-bold text-foreground bg-slate-100 text-sm border-r-2 border-slate-300">
                CEF DETAILS
              </th>
            ...
            <tr className="bg-slate-50">
              <th className="h-7 px-1.5 text-center sticky left-0 z-30 bg-slate-50 border-r border-slate-200 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)]">
                <Tooltip delayDuration={200}>
                  ...
                  >
                    <p className="text-center">Click the star icon to add CEFs to your favorites</p>
                  </TooltipContent>
                </Tooltip>
              </th>
              ...
              <th className="h-7 px-1.5 text-left bg-slate-50 text-xs">
                <SortButton field="navSymbol" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>NAV</SortButton>
              </th>
              <th className="h-7 px-1.5 text-left bg-slate-50 text-xs">
              ...
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="openDate" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>OPEN</SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="dividendHistory" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>
                  <div className="whitespace-normal leading-tight">DIV<br />HISTO</div>
                </SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
              ...
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="marketPrice" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>MP</SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="nav" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>NAV</SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="lastDividend" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>
                  <div className="whitespace-normal leading-tight">Last<br />Div</div>
                </SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="numPayments" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>#</SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="yearlyDividend" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>
                  <div className="whitespace-normal leading-tight">Yrly<br />Div</div>
                </SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="forwardYield" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>
                  <div className="whitespace-normal leading-tight">F<br />Yield</div>
                </SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="premiumDiscount" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>
                  <div className="whitespace-normal leading-tight">Prem/<br />Disc</div>
                </SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="fiveYearZScore" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>
                  <div className="whitespace-normal leading-tight">5 Yr<br />Z-Score</div>
                </SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <SortButton field="navTrend6M" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>
                  <div className="whitespace-normal leading-tight">6 Mo<br />NAV Trend</div>
                </SortButton>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
              ...
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs">
                <Tooltip delayDuration={200}>
                  ...
                  >
                    <p>Dividend Volatility Index is computed using the Coefficient of Variation (CV) with Adjusted Dividends</p>
                  </TooltipContent>
                </Tooltip>
              </th>
              <th className="h-7 px-1.5 text-center bg-slate-50 text-xs border-r-2 border-slate-300">
                <SortButton field="weightedRank" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>Rank</SortButton>
              </th>
              {returnColumns.map((col) => (
                <th key={col.key} className="h-7 px-1.5 text-center bg-slate-50 text-xs">
          ...
          <tbody>
            {sortedCEFs.map((cef) => (
              ...
              >
                ...
                <td className={`py-1 px-1.5 align-middle text-center tabular-nums text-xs font-medium ${cef.ipoPrice && cef.marketPrice && cef.marketPrice > cef.ipoPrice ? 'bg-green-100 text-green-700' : ''
                  }`}>
                ...
                <td className="py-1 px-1.5 align-middle text-center tabular-nums text-xs font-medium text-foreground">
                  {cef.marketPrice != null ? formatCurrency(cef.marketPrice) : 'N/A'}
                </td>
                <td className="py-1 px-1.5 align-middle text-center tabular-nums text-xs font-medium text-foreground">
                ...
                <td className="py-1 px-1.5 align-middle text-center tabular-nums text-xs text-muted-foreground">
                  {cef.yearlyDividend != null ? formatCurrency(cef.yearlyDividend) : 'N/A'}
                </td>
                ...
                <td className={`py-1 px-1.5 align-middle text-center tabular-nums text-xs font-medium ${cef.premiumDiscount != null && cef.premiumDiscount >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                  {cef.premiumDiscount != null ? formatPercentage(cef.premiumDiscount) : 'N/A'}
                </td>
                <td className={`py-1 px-1.5 align-middle text-center tabular-nums text-xs font-medium ${cef.fiveYearZScore != null && cef.fiveYearZScore >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                  {cef.fiveYearZScore != null ? cef.fiveYearZScore.toFixed(2) : 'N/A'}
                </td>
                <td className={`py-1 px-1.5 align-middle text-center tabular-nums text-xs font-medium ${cef.navTrend6M != null && cef.navTrend6M >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
